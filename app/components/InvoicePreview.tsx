...
// Componente InvoicePreview actualizado
    ...
    return (
      <div id="invoice-preview" className="bg-white w-[210mm] min-h-[297mm] p-[20mm] mx-auto">
        <div className="flex flex-col min-h-full">
          ...
          {/* Invoice Items - Layout Mejorado */}
          <div className="flex-grow">
            <table className="w-full mb-8">
              <thead>
                <tr className="border-b border-black">
                  <th className="text-left py-3 pr-8 font-normal w-2/5">Descripción</th>
  ...
  return (
    <div id="invoice-preview" className="bg-white w-[210mm] min-h-[297mm] p-[20mm] mx-auto">
      <div className="flex flex-col min-h-full">
        ...
        {/* Invoice Items - Layout Original */}
        <div className="flex-grow">
          <table className="w-full mb-16">
            <thead>
              <tr className="border-b border-black">
                <th className="text-left py-2 pr-6 font-normal">Descripción</th>
                <th className="text-left py-2 pr-6 font-normal">Cliente</th>
                <th className="text-left py-2 pr-6 font-normal">Unidades</th>
                {hasPrices && <th className="text-left py-2 pr-6 font-normal">Precio x h</th>}
                <th className="text-right py-2 font-normal">Total</th>
              </tr>
            </thead>
            <tbody>
              {items.map((item, index) => (
                <tr key={index}>
                  <td className="py-2 pr-6">{item.description}</td>
                  <td className="py-2 pr-6">{item.client}</td>
                  <td className="py-2 pr-6">{item.units}</td>
                  {hasPrices && <td className="py-2 pr-6">{item.price ? `${item.price} EUR` : ""}</td>}
                  <td className="text-right py-2">{item.total ? `${item.total} EUR` : ""}</td>
                </tr>
              ))}
            </tbody>
